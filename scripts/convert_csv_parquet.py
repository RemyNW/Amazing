...
def process_file_pyarrow(file):
    try:
        ...
        print(f"  Traitement de {file} ({file_size:.1f} MB)")

        # Skip if already exists
        if os.path.exists(output_path):
            return f"Fichier déjà existant, ignoré: {output_path}"

        # PyArrow CSV reading options for better memory management
        read_options = pa_csv.ReadOptions(
            use_threads=True,
            block_size=10<<20,  # 10MB blocks
        )
        ...
        schema = pa.schema([
            ...
            ("category_code", pa.dictionary(pa.int16(), pa.string())),
            ("brand", pa.dictionary(pa.int16(), pa.string())),
            ("price", pa.float32()),
            ("user_id", pa.int64()),
        ...
        convert_options = pa_csv.ConvertOptions(
            check_utf8=False,  # Faster, less memory
            column_types=schema,
        )

        # Read CSV with PyArrow (handles .gz automatically)
        ...
        print(f"    Colonnes: {len(table.column_names)}, Lignes: {len(table)}")

        # Write to Parquet with compression
        print(f"    Écriture en Parquet...")
        pq.write_table(
            table,
            output_path,
            compression='zstd',
        ...
        return f"✓ {file_name}.parquet ({output_size:.1f} MB, compression: {compression_ratio:.1f}x)"

    except Exception as e:
        gc.collect()
        return f"✗ Erreur lors du traitement de {file}: {str(e)}"
