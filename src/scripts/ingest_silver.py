...
# Add the parent directory of `silver` to sys.path
# current_dir = os.path.dirname(__file__)  # silver/
...
def identify_purchase_users(months: List[str], path_data: str) -> Tuple[Set[str], Set[str]]:
    ...
    for month in tqdm(months, desc="Processing months"):
        try:
            ...
            file_path = f"{path_data}/bronze/{month}.parquet"

            # Load only necessary columns for efficiency
            df_month = pl.read_parquet(
                file_path,
                # schema_overrides={
...
def process_and_save_filtered_data(months: List[str], purchase_user_ids: Set[str], path_data: str) -> None:
    """
    ...
    Args:
        ...
        path_data: Path to the data directory
    """
    print(f"\nStep 2: Processing and saving data ...")
    ...
    for month in tqdm(months, desc="Processing months"):
        try:
            ...
            df_month = pl.read_parquet(
                file_path,
                 columns=[
                    "user_id", "product_id", "category_id", "price", "event_type",
                    "category_code", "brand", "user_session", "event_time"
                ]
                # schema_overrides={
                #     'user_id': pl.Categorical,
...
def main():
    ...
    process_and_save_filtered_data(months, purchase_user_ids, PATH_DATA)
